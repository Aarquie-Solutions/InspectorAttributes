...
namespace AarquieSolutions.InspectorAttributes.Editor
{
	...
	public class ExtendedInspector : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			bool anyNaughtyAttribute = _serializedProperties.Any(p => PropertyUtility.GetAttribute<IInspectorAttribute>(p) != null);
		...
		protected void DrawSerializedProperties()
		{
			...
			foreach (var property in GetNonGroupedProperties(_serializedProperties))
			{
				...
				{
					InspectorAttributeEditorGUI.PropertyField_Layout(property, includeChildren: true);
				}
			}
			...
			foreach (var group in GetGroupedProperties(_serializedProperties))
			{
				...
				InspectorAttributeEditorGUI.BeginBoxGroup_Layout(group.Key);
				foreach (var property in visibleProperties)
				{
					InspectorAttributeEditorGUI.PropertyField_Layout(property, includeChildren: true);
				}

				InspectorAttributeEditorGUI.EndBoxGroup_Layout();
			}
			...
			foreach (var group in GetFoldoutProperties(_serializedProperties))
			{
				...
				if (_foldouts[group.Key].Value)
				{
					foreach (var property in visibleProperties)
					{
						InspectorAttributeEditorGUI.PropertyField_Layout(property, true);
					}
				}
			}
		...
		protected void DrawNonSerializedFields(bool drawHeader = false)
		{
			if (_nonSerializedFields.Any())
			{
				if (drawHeader)
				{
					...
					InspectorAttributeEditorGUI.HorizontalLine(
				...
				{
					InspectorAttributeEditorGUI.NonSerializedField_Layout(serializedObject.targetObject, field);
				}
			}
		}
		...
		protected void DrawNativeProperties(bool drawHeader = false)
		{
			if (_nativeProperties.Any())
			{
				...
				{
					InspectorAttributeEditorGUI.NativeProperty_Layout(serializedObject.targetObject, property);
				}
			}
		}
		...
		protected void DrawButtons(bool drawHeader = false)
		{
			if (_methods.Any())
			{
				...
				{
					InspectorAttributeEditorGUI.Button(serializedObject.targetObject, method);
				}
			}
		}
